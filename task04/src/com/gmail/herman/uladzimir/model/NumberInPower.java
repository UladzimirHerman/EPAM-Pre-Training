...
public class NumberInPower {
    ...
    public static double countNumberInPowerRecursively(double number, int power)
            throws NumberIsNullException {

        if (NumberUtil.isNotNullNumber(number)) {

            if (Math.abs(power) > 1) {
                return power > 1 ?
                        number * countNumberInPowerRecursively(number, power - 1) :
    ...
    public static double countNumberInPowerIteratively(double number, int power)
            throws NumberIsNullException {
